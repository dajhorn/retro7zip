...
SET WATCOM_VERSION=unknown
FOR /F "delims=" %%A IN ('wcc386 ^| findstr /B "Version"') DO SET "WATCOM_VERSION=%%A"
ECHO #define WATCOM_VERSION      "Open Watcom %WATCOM_VERSION%" >"%VERSION_H%"
...
git log HEAD -n1 --format="%FORMAT%" >>"%VERSION_H%"
GOTO END
